...
# 출력 내용을 파일로 저장할 예정입니다.
# 따라서, 출력이 저장될 폴더 경로를 지정하였습니다.
result_path = './result'
if not os.path.exists(result_path):
    os.mkdir(result_path)
sys.stdout = open(result_path + '/result.txt', 'w', encoding='UTF-8')
...
print_value('X_train.head()')
print_value('X_test.head()')
print_value('y_train.head()')
print_value('y_test.head()')
...
print_value("wine_knn_5.score(X_train, y_train)")
...
print_value("wine_knn_5.score(X_test, y_test)")
...
train_score = wine_knn_3.score(X_train, y_train)
print("Score_with_train_set: {}%".format(round(train_score*100, 2)))
print_value("wine_knn_3.score(X_train, y_train)")

# Model에 대하여 Test Set의 예측값을 출력 합니다.
test_score = wine_knn_3.score(X_test, y_test)
print("Score_with_train_set: {}%".format(round(test_score*100, 2)))
print_value("wine_knn_3.score(X_test, y_test)")
